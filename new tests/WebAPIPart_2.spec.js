...
test.beforeAll(async ({ browser }) => {
    ...
    // Now save login session
    await context.storageState({ path: 'WebAPIPart_2_Login_state.json' });
    webConetext = await browser.newContext({ storageState: 'WebAPIPart_2_Login_state.json' });
});

test("E2E Purchase Flow", async () => {

    const productName = "ZARA COAT 3";
    const page = await webConetext.newPage();
    await page.goto("https://rahulshettyacademy.com/client");
